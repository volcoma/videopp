namespace gli
{
	...
	inline storage_linear::size_type storage_linear::image_offset(extent3d const& Coord, extent3d const& Extent) const
	{
		GLI_ASSERT(glm::all(glm::lessThan(Coord, Extent)));
        auto x = Coord.x;
        auto y = Coord.y * Extent.x;
        auto z = Coord.z * Extent.x * Extent.y;
		...
		extent_type const& BlockCount)
	{
		...
		for(size_t BlockIndexY = 0, BlockCountY = BlockCount.y; BlockIndexY < BlockCountY; ++BlockIndexY)
		{
            ...
            gli::size_t const OffsetSrc = StorageSrc.image_offset(BlockIndexSrc + BlockIndex, StorageSrc.extent(LevelSrc) / StorageSrc.block_extent()) * StorageSrc.block_size();;
            gli::size_t const OffsetDst = this->image_offset(BlockIndexDst + BlockIndex, this->extent(LevelDst) / this->block_extent()) * this->block_size();
